...
async def get_all_memory_items():
    return get_knowledge_manager().get_all_memories()
...
async def add_memory_item(item: MemoryItem):
    try:
        ...
        item_id = get_knowledge_manager().add_memory(
...
async def delete_memory_item(item_id: int):
    success = get_knowledge_manager().delete_memory(item_id)
    if not success:
...
async def update_memory_item(item_id: int, item: UpdateMemoryRequest):
    success = get_knowledge_manager().update_memory(item_id, item.content)
    if not success:
...
async def get_memory_candidates(include_promoted: bool = False, limit: int = 200):
    return get_knowledge_manager().get_memory_candidates(include_promoted=include_promoted, limit=limit)
...
async def promote_memory_candidate(candidate_id: int):
    memory_id = get_knowledge_manager().promote_memory_candidate(candidate_id)
    if not memory_id:
...
async def delete_memory_candidate(candidate_id: int):
    success = get_knowledge_manager().delete_memory_candidate(candidate_id)
    if not success:
...
async def get_all_worldbook_items():
    return get_knowledge_manager().get_all_world_book_entries()
...
async def add_worldbook_item(item: WorldBookItem):
    try:
        item_id = get_knowledge_manager().add_world_book_entry(
...
async def update_worldbook_item(item_id: int, item: WorldBookItem):
    try:
        success = get_knowledge_manager().update_world_book_entry(
            entry_id=item_id,
            keywords=item.keywords,
            content=item.content,
...
async def delete_worldbook_item(item_id: int):
    success = get_knowledge_manager().delete_world_book_entry(item_id)
    if not success:
        raise HTTPException(status_code=404, detail="World book item not found")
    return Response(status_code=204)