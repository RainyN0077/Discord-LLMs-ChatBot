...
async def simulate_debugger_run(request: DebuggerRequest):
    ...
    try:
        ...
        guild_id = int(request.guild_id) if request.guild_id else None
    except ValueError:
        raise HTTPException(status_code=400, detail="user_id, channel_id and guild_id must be numeric strings.")
    ...
    try:
        ...
        async for response_type, data in llm_provider.get_response_stream(llm_messages):
            if response_type == "partial":
                llm_response_chunks.append(str(data))
            elif response_type == "final":
                llm_response_chunks = [str(data)]
    except Exception as e:
        logger.warning(f"Debugger simulation LLM invocation failed: {e}")
        llm_error = str(e)
