<script>
  ...
  async function fetchLogs() {
    try {
        ...
        rawLogs = res.ok ? await res.text() : 'Error loading logs... Could not connect to backend.';
    } catch (e) {
        console.error("Failed to fetch logs:", e);
        rawLogs = 'Error loading logs... Could not connect to backend.';
    }
  }
  ...
  async function handleSave() {
    ...
    const rolesAsObject = config.role_based_config.reduce((acc, r) => {
        if (r.id) acc[r.id] = { 
            ...
            enable_message_limit: r.enable_message_limit,
  ...
  function addRoleConfig() { config.role_based_config = [...config.role_based_config, { id: '', title: '', prompt: '', enable_message_limit: false, message_limit: 0, message_refresh_minutes: 60, enable_char_limit: false, char_limit: 0, char_refresh_minutes: 60, display_color: '#ffffff' }]; }
...
<div class="page-container">
    <main>
        ...
        <div class="card">
            ...
            <select id="llm-provider" bind:value={config.llm_provider}><option value="openai">{$t('llmProvider.providers.openai')}</option><option value="google">{$t('llmProvider.providers.google')}</option><option value="anthropic">{$t('llmProvider.providers.anthropic')}</option></select>
        ...
        <div class="card">
            ...
            <div class="radio-group"><label><input type="radio" bind:group={config.context_mode} value={'none'}> {$t('contextControl.modes.none')}</label><label><input type="radio" bind:group={config.context_mode} value={'channel'}> {$t('contextControl.modes.channel')}</label><label><input type="radio" bind:group={config.context_mode} value={'memory'}> {$t('contextControl.modes.memory')}</label></div>
             {#if config.context_mode !== 'none'}
            <div class="context-settings">
                <p class="info">{config.context_mode === 'channel' ? $t('contextControl.channelModeInfo') : $t('contextControl.memoryModeInfo')}</p>
                 <div class="control-grid"><label for="context-messages">{$t('contextControl.historyLimit')}</label><div class="slider-container"><input type="range" id="context-messages" min="0" max="50" step="5" bind:value={config[config.context_mode + '_context_settings'].message_limit}><span>{config[config.context_mode + '_context_settings'].message_limit} {$t('contextControl.messages')}</span></div><label for="context-chars">{$t('contextControl.charLimit')}</label><input type="number" id="context-chars" placeholder={$t('contextControl.charLimitPlaceholder')} bind:value={config[config.context_mode + '_context_settings'].char_limit}></div>
            </div>
            {:else}<div class="context-settings"><p class="info">{$t('contextControl.noneModeInfo')}</p></div>{/if}
        </div>
        <div class="card">
            <h2>{$t('userPortrait.title')}</h2><p class="info">{$t('userPortrait.info')}</p>
            <div class="list-container">{#each (config.user_personas || []) as persona, i (persona.id || i)}<div class="list-item"><div class="list-item-main"><input class="id-input" type="text" placeholder={$t('userPortrait.userId')} bind:value={persona.id}><input class="nickname-input" type="text" placeholder={$t('userPortrait.customNicknamePlaceholder')} bind:value={persona.nickname}><textarea class="prompt-input" rows="2" placeholder={$t('userPortrait.personaPrompt')} bind:value={persona.prompt}></textarea></div><button class="remove-btn" on:click={() => removePersona(i)} title="Remove">×</button></div>{/each}</div>
            <button class="add-btn" on:click={addPersona}>{$t('userPortrait.add')}</button>
        </div>
        <div class="card">
            <h2>{$t('roleConfig.title')}</h2><p class="info">{$t('roleConfig.info')}</p>
            <div class="list-container">{#each (config.role_based_config || []) as role, i (role.id || i)}<div class="list-item complex-item">
                <div class="list-item-main very-wide-grid">
                    <input class="id-input" type="text" placeholder={$t('roleConfig.roleId')} bind:value={role.id}>
                    <input class="nickname-input" type="text" placeholder={$t('roleConfig.roleTitle')} bind:value={role.title}>
                    <textarea class="prompt-input" rows="3" placeholder={$t('roleConfig.rolePrompt')} bind:value={role.prompt}></textarea>
                    <div class="limit-control-group"><label class="toggle-switch"><input type="checkbox" bind:checked={role.enable_message_limit}><span class="slider"></span>{$t('roleConfig.enableMsgLimit')}</label><div class="limit-group" class:disabled={!role.enable_message_limit}><input type="number" min="0" placeholder="0" bind:value={role.message_limit} disabled={!role.enable_message_limit}><span>/</span><input type="number" min="1" placeholder="60" bind:value={role.message_refresh_minutes} disabled={!role.enable_message_limit}><span class="unit">{$t('roleConfig.minutes')}</span></div></div>
                    <div class="limit-control-group"><label class="toggle-switch"><input type="checkbox" bind:checked={role.enable_char_limit}><span class="slider"></span>{$t('roleConfig.enableCharLimit')}</label><div class="limit-group" class:disabled={!role.enable_char_limit}><input type="number" min="0" placeholder="0" bind:value={role.char_limit} disabled={!role.enable_char_limit}><span>/</span><input type="number" min="1" placeholder="60" bind:value={role.char_refresh_minutes} disabled={!role.enable_char_limit}><span class="unit">{$t('roleConfig.minutes')}</span></div></div>
                    <div class="preview-section"><label>{$t('roleConfig.previewTitle')}</label><div class="quota-preview"><div class="preview-header" style="color: {role.display_color};">{$t('roleConfig.previewHeader')}</div><div class="preview-field"><span class="field-name">{$t('roleConfig.msgLimit')}</span><span class="field-value" style="color: {role.display_color};">{#if role.enable_message_limit}{role.message_limit - Math.floor(role.message_limit / 3)}/{role.message_limit > 0 ? role.message_limit : '∞'}{:else}{$t('roleConfig.disabled')}{/if}</span></div><div class="preview-field"><span class="field-name">{$t('roleConfig.charLimit')}</span><span class="field-value" style="color: {role.display_color};">{#if role.enable_char_limit}{role.char_limit - Math.floor(role.char_limit / 4)}/{role.char_limit > 0 ? role.char_limit : '∞'}{:else}{$t('roleConfig.disabled')}{/if}</span></div></div></div>
        ...
        <div class="card">
            ...
            <label for="model-name">{$t('defaultBehavior.modelName')}</label><input id="model-name" type="text" placeholder={$t(`defaultBehavior.modelPlaceholders.${config.llm_provider}`)} bind:value={config.model_name}>
            <label for="system-prompt">{$t('defaultBehavior.systemPrompt')}</label><textarea id="system-prompt" rows="4" placeholder={$t('defaultBehavior.systemPromptPlaceholder')} bind:value={config.system_prompt}></textarea>
            <label for="trigger-keywords">{$t('defaultBehavior.triggerKeywords')}</label><input id="trigger-keywords" type="text" placeholder={$t('defaultBehavior.triggerKeywordsPlaceholder')} bind:value={keywordsInput}>
            <label>{$t('defaultBehavior.responseMode')}</label><div class="radio-group"><label><input type="radio" bind:group={config.stream_response} value={true}> {$t('defaultBehavior.modes.stream')}</label><label><input type="radio" bind:group={config.stream_response} value={false}> {$t('defaultBehavior.modes.nonStream')}</label></div>
        </div>
        <div class="card dark-theme">
            <h2>{$t('customParams.title')}</h2>
            <div class="list-container">{#each (config.custom_parameters || []) as param, i (i)}<div class="list-item param-item">
                <input class="param-input" type="text" placeholder={$t('customParams.paramName')} bind:value={param.name}>
                <select class="param-select" bind:value={param.type} on:change={() => handleParamTypeChange(param)}><option value="text">{$t('customParams.types.text')}</option><option value="number">{$t('customParams.types.number')}</option><option value="boolean">{$t('customParams.types.boolean')}</option><option value="json">{$t('customParams.types.json')}</option></select>
                {#if param.type === 'text'}<input class="param-input" type="text" placeholder={$t('customParams.paramValue')} bind:value={param.value}>
                {:else if param.type === 'number'}<input class="param-input" type="number" step="0.01" placeholder={$t('customParams.paramValue')} bind:value={param.value}>
                {:else if param.type === 'boolean'}<select class="param-select wide" bind:value={param.value}><option value="true">True</option><option value="false">False</option></select>
                {:else if param.type === 'json'}<textarea class="param-input param-textarea" rows="1" placeholder={$t('customParams.paramValue')} bind:value={param.value}></textarea>{/if}
                <button class="remove-btn" on:click={() => removeParameter(i)} title="Remove">×</button>
            </div>{/each}</div><button class="add-btn" on:click={addParameter}>{$t('customParams.add')}</button>
        </div>
        <div class="card">
            <h2>{$t('sessionManagement.title')}</h2><p class="info">{$t('sessionManagement.info')}</p>
