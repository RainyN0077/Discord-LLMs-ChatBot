<script>
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!config.custom_parameters) config.custom_parameters = [];
  ...
  function addRoleConfig() { config.role_based_config = [...config.role_based_config, { id: '', title: '', prompt: '', message_limit: 0, message_refresh_minutes: 60, char_limit: 0, char_refresh_minutes: 60 }]; }
...
<div class="page-container">
    <main>
                ...
                <p class="info">{config.context_mode === 'channel' ? $t('contextControl.channelModeInfo') : $t('contextControl.memoryModeInfo')}</p>
                 <div class="control-grid">
                    ...
                    <div class="slider-container">
                        <input type="range" id="context-messages" min="0" max="50" step="5" bind:value={config[config.context_mode + '_context_settings'].message_limit}>
                        <span>{config[config.context_mode + '_context_settings'].message_limit} {$t('contextControl.messages')}</span>
                    </div>
                    <label for="context-chars">{$t('contextControl.charLimit')}</label>
                    <input type="number" id="context-chars" placeholder={$t('contextControl.charLimitPlaceholder')} bind:value={config[config.context_mode + '_context_settings'].char_limit}>
                </div>
            </div>
            {:else}
              <div class="context-settings"><p class="info">{$t('contextControl.noneModeInfo')}</p></div>
            {/if}
        </div>
        ...
        <div class="card">
            <h2>{$t('roleConfig.title')}</h2>
            <p class="info">{$t('roleConfig.info')}</p>
            <div class="list-container">
                {#if config.role_based_config}
                    {#each config.role_based_config as role, i (role.id || i)}
                        <div class="list-item complex-item">
                            <div class="list-item-main wide-grid">
                                ...
                                <div class="limit-group">
                                    <label>{$t('roleConfig.msgLimit')}</label>
                                    <input type="number" min="0" placeholder="0" bind:value={role.message_limit}>
                                    <span>/</span>
                                    <input type="number" min="0" placeholder="60" bind:value={role.message_refresh_minutes}>
                                    <span class="unit">{$t('roleConfig.minutes')}</span>
                                </div>
                                <div class="limit-group">
                                    <label>{$t('roleConfig.charLimit')}</label>
                                    <input type="number" min="0" placeholder="0" bind:value={role.char_limit}>
                                    <span>/</span>
                                    <input type="number" min="0" placeholder="60" bind:value={role.char_refresh_minutes}>
                                    <span class="unit">{$t('roleConfig.minutes')}</span>
                                </div>
                            </div>
                            <button class="remove-btn" on:click={() => removeRoleConfig(i)} title="Remove">×</button>
                        </div>
                    {/each}
                {/if}
            </div>
    ...
    <aside class="log-viewer">
        ...
        <div class="log-controls">
            <div class="log-filter-group"><span>{$t('logViewer.filterLevel')}:</span>{#each logLevels as level}<button class:active={logLevelFilter === level} on:click={() => {logLevelFilter = level; autoScroll = true;}}>{level}</button>{/each}</div>
            <label class="autoscroll-toggle"><input type="checkbox" bind:checked={autoScroll}><span>{$t('logViewer.autoscroll')}</span></label>
        </div>
...
:root {
    ...
    --bg-color: #f7f9fc; --card-bg: #ffffff; --text-color: #2c3e50; --text-light: #7f8c8d;
    --primary-color: #3498db; --primary-hover: #2980b9; --border-color: #e4e7eb;
    --success-bg: #e0f2f1; --success-text: #00796b; --error-bg: #fce4ec; --error-text: #c2185b;
    --info-bg: #e1f5fe; --info-text: #0277bd; --save-color: #2ecc71; --save-hover: #27ae60;
    --shadow: 0 4px 6px rgba(0, 0, 0, 0.05);
}
body { background-color: var(--bg-color); color: var(--text-color); margin: 0; padding: 2rem; -webkit-font-smoothing: antialiased; }
.page-container { display: flex; align-items: flex-start; gap: 2rem; max-width: 1600px; margin: 0 auto; }
main { flex: 1; min-width: 600px; display: flex; flex-direction: column; gap: 2rem; }
h1 { color: var(--primary-color); text-align: center; font-weight: 300; letter-spacing: 1px; margin-bottom: 0; }
h2 { color: var(--text-color); font-weight: 600; border-bottom: 1px solid var(--border-color); padding-bottom: 0.5rem; margin-top: 0; }
.card { background: var(--card-bg); border-radius: 12px; padding: 1.5rem 2rem; box-shadow: var(--shadow); display: flex; flex-direction: column; gap: 1.2rem; }
label { font-weight: 500; color: var(--text-light); }
input, textarea, select { width: 100%; padding: 0.8rem 1rem; border: 1px solid var(--border-color); border-radius: 8px; font-size: 1rem; box-sizing: border-box; background-color: #fcfdfe; transition: all 0.2s ease; }
...
.dark-theme input, .dark-theme select, .dark-theme textarea { background-color: #40414f; border: 1px solid #565869; color: #ececf1; }
.dark-theme input::placeholder, .dark-theme textarea::placeholder { color: #8e8ea0; }
.dark-theme .remove-btn { color: #acacbe; } .dark-theme .remove-btn:hover { background-color: rgba(239, 68, 68, 0.2); color: #ef4444; }
.dark-theme .add-btn { color: #ececf1; background-color: #40414f; border: 1px solid #565869; } .dark-theme .add-btn:hover { background-color: #4d4e5a; }
...
.log-viewer { width: 600px; flex-shrink: 0; background-color: var(--card-bg); border-radius: 12px; padding: 1rem 1.5rem; box-shadow: var(--shadow); position: sticky; top: 2rem; height: calc(100vh - 4rem); display: flex; flex-direction: column; }
.log-viewer h2 { font-size: 1.25rem; }
.log-controls { display: flex; justify-content: space-between; align-items: center; gap: 1rem; margin-bottom: 1rem; flex-shrink: 0; flex-wrap: wrap; }
.log-filter-group { display: flex; gap: 0.5rem; align-items: center; }
.log-filter-group span { color: var(--text-light); font-size: 0.9rem; }
.log-filter-group button { background: #f0f2f5; color: #555; padding: 0.3rem 0.8rem; font-size: 0.85rem; border: 1px solid var(--border-color); }
.log-filter-group button.active { background-color: var(--primary-color); color: white; border-color: var(--primary-color); }
.autoscroll-toggle { display: flex; align-items: center; gap: 0.5rem; color: var(--text-light); font-size: 0.9rem; cursor: pointer; user-select: none; }
.autoscroll-toggle input { width: auto; }
.log-output-wrapper { flex-grow: 1; overflow: hidden; background-color: #1e1e1e; border-radius: 8px; position: relative; }
.log-output-wrapper pre { height: 100%; margin: 0; overflow-y: auto; padding: 1rem; box-sizing: border-box; font-family: 'Fira Code', 'Courier New', monospace; font-size: 0.8rem; line-height: 1.6; color: #d4d4d4; white-space: pre-wrap; word-break: break-all; }
.log-output-wrapper code { display: block; }
.log-line.INFO { color: #81c784; }
.log-line.WARNING { color: #ffd54f; }
.log-line.ERROR { color: #e57373; }
.log-line.CRITICAL { color: #ff8a65; font-weight: bold; }
.log-line.UNKNOWN { color: #90a4ae; }
.wide-grid { grid-template-columns: 1fr 1fr; grid-template-rows: auto auto auto; gap: 0.75rem 1rem; }
.wide-grid .id-input { grid-column: 1 / 2; }
.wide-grid .nickname-input { grid-column: 2 / 3; }
.wide-grid .prompt-input { grid-column: 1 / 3; grid-row: 2 / 3; }
.wide-grid .limit-group { grid-row: 3 / 4; display: flex; align-items: center; gap: 0.5rem;}
.wide-grid .limit-group label { margin-right: 0.5rem; white-space: nowrap; color: var(--text-color); font-size: 0.9em; }
.wide-grid .limit-group input { padding: 0.5rem; text-align: center; }
.wide-grid .limit-group span { color: var(--text-light); }
.wide-grid .limit-group .unit { font-size: 0.8em; }

@media (max-width: 1400px) {
    .page-container { flex-direction: column; align-items: stretch; }
    main { min-width: unset; }
    .log-viewer { width: auto; position: relative; top: 0; height: 70vh; margin-top: 2rem; }
}
</style>
