<script>
  ...
  let config = {
    ...
    trigger_keywords: [], stream_response: true, user_personas: [], role_based_config: [], 
    ...
    custom_parameters: [], plugins: [], api_secret_key: ''
  };
  ...
  function applyFont(fontDataUrl, fontName) {
    ...
    showStatus($t('uiSettings.font.loadSuccess', { fontName }), 'success');
  }
  ...
  function handleFontFileSelect(event) {
    ...
    reader.onload = (e) => {
        ...
        try {
            ...
            console.error(error);
            showStatus($t('uiSettings.font.localStorageError'), 'error');
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        config.scoped_prompts = {
            guilds: Object.entries(loadedConfig.scoped_prompts?.guilds || {}).map(([id, s]) => ({ id, ...s })),
            channels: Object.entries(loadedConfig.scoped_prompts?.channels || {}).map(([id, s]) => ({ id, ...s }))
        };
  ...
  async function handleSave() {
    ...
    const scopedPromptsAsObject = {
        guilds: (config.scoped_prompts.guilds || []).reduce((acc, s) => { if(s.id) acc[s.id] = { enabled: s.enabled, mode: s.mode, prompt: s.prompt }; return acc; }, {}),
        channels: (config.scoped_prompts.channels || []).reduce((acc, s) => { if(s.id) acc[s.id] = { enabled: s.enabled, mode: s.mode, prompt: s.prompt }; return acc; }, {})
    };
    ...
    const finalPlugins = (config.plugins || []).map(p => ({ ...p, triggers: Array.isArray(p.triggers) ? p.triggers : p.triggers.split(',').map(t => t.trim()).filter(Boolean) }));

    const finalConfig = { 
        ...config, 
        user_personas: personasAsObject, 
        role_based_config: rolesAsObject, 
        scoped_prompts: scopedPromptsAsObject,
        custom_parameters: finalParams, 
        trigger_keywords: keywordsInput.split(',').map(k => k.trim()).filter(Boolean),
    ...
    try {
      ...
      const loadedConfig = await (await fetch('/api/config')).json();
      config = {...config, ...loadedConfig};
      config.user_personas = Object.entries(config.user_personas || {}).map(([id, p]) => ({ id, ...p }));
      config.role_based_config = Object.entries(config.role_based_config || {}).map(([id, r]) => ({ id, ...r }));
      config.scoped_prompts = {
  ...
  function addPlugin() { config.plugins = [...(config.plugins || []), { name: 'New Plugin', enabled: true, trigger_type: 'command', triggers: [], action_type: 'http_request', injection_mode: 'override', http_request_config: { url: '', method: 'GET', headers: '{}', body_template: '{}' }, llm_prompt_template: 'Based on the user query "{user_input}", summarize the following API data:\n\n{api_result}' }]; }
  function removePlugin(index) { config.plugins.splice(index, 1); config.plugins = config.plugins; }
  function addScopedPrompt(type) { config.scoped_prompts[type] = [...(config.scoped_prompts[type] || []), { id: '', enabled: true, mode: 'append', prompt: '' }]; }
  function removeScopedPrompt(type, index) { config.scoped_prompts[type].splice(index, 1); config.scoped_prompts[type] = config.scoped_prompts[type]; }
    ...
    <main>
        ...
        <div class="card">
            ...
            <div class="api-key-container">
                <input id="api-key-display" type="text" readonly bind:value={config.api_secret_key} placeholder={$t('globalConfig.apiKeyUnavailable')}>
                <button on:click={() => navigator.clipboard.writeText(config.api_secret_key)} title={$t('globalConfig.copy')}>
        ...
        <div class="card">
            <h2>{$t('scopedPrompts.channel.title')}</h2>
            <p class="info">{$t('scopedPrompts.channel.info')}</p>
            <div class="list-container">
                {#each (config.scoped_prompts.channels || []) as item, i}
                <div class="list-item complex-item">
                     <div class="list-item-main scoped-prompt-grid">
                        <div class="cell-id"><label>{$t('scopedPrompts.channel.id')}</label><input type="text" placeholder={$t('scopedPrompts.channel.idPlaceholder')} bind:value={item.id}></div>
                        <div class="cell-toggle"><label class="toggle-switch"><input type="checkbox" bind:checked={item.enabled}><span class="slider"></span>{$t('scopedPrompts.enabled')}</label></div>
                        <div class="cell-mode"><label>{$t('scopedPrompts.mode.title')}</label><div class="radio-group small"><label><input type="radio" bind:group={item.mode} value={'override'}> {$t('scopedPrompts.mode.override')}</label><label><input type="radio" bind:group={item.mode} value={'append'}> {$t('scopedPrompts.mode.append')}</label></div></div>
                        <div class="cell-prompt"><label>{$t('scopedPrompts.channel.prompt')}</label><textarea rows="3" placeholder={item.mode === 'override' ? $t('scopedPrompts.channel.overridePlaceholder') : $t('scopedPrompts.channel.appendPlaceholder')} bind:value={item.prompt}></textarea></div>
                    </div>
                    <button class="remove-btn" on:click={() => removeScopedPrompt('channels', i)} title="Remove">×</button>
                </div>
                {/each}
            </div>
            <button class="add-btn" on:click="{() => addScopedPrompt('channels')}">{$t('scopedPrompts.channel.add')}</button>
        </div>
        
        <div class="card">
            <h2>{$t('scopedPrompts.guild.title')}</h2>
            <p class="info">{$t('scopedPrompts.guild.info')}</p>
            <div class="list-container">
                {#each (config.scoped_prompts.guilds || []) as item, i}
                <div class="list-item complex-item">
                    <div class="list-item-main scoped-prompt-grid">
                        <div class="cell-id"><label>{$t('scopedPrompts.guild.id')}</label><input type="text" placeholder={$t('scopedPrompts.guild.idPlaceholder')} bind:value={item.id}></div>
                        <div class="cell-toggle"><label class="toggle-switch"><input type="checkbox" bind:checked={item.enabled}><span class="slider"></span>{$t('scopedPrompts.enabled')}</label></div>
                        <div class="cell-mode"><label>{$t('scopedPrompts.mode.title')}</label><div class="radio-group small"><label><input type="radio" bind:group={item.mode} value={'override'}> {$t('scopedPrompts.mode.override')}</label><label><input type="radio" bind:group={item.mode} value={'append'}> {$t('scopedPrompts.mode.append')}</label></div></div>
                        <div class="cell-prompt"><label>{$t('scopedPrompts.guild.prompt')}</label><textarea rows="3" placeholder={item.mode === 'override' ? $t('scopedPrompts.guild.overridePlaceholder') : $t('scopedPrompts.guild.appendPlaceholder')} bind:value={item.prompt}></textarea></div>
                    </div>
                    <button class="remove-btn" on:click={() => removeScopedPrompt('guilds', i)} title="Remove">×</button>
                </div>
                {/each}
            </div>
            <button class="add-btn" on:click="{() => addScopedPrompt('guilds')}">{$t('scopedPrompts.guild.add')}</button>
        </div>
        ...
        <div class="card dark-theme">
            ...
            <div class="list-container">
                ...
                <div class="list-item complex-item plugin-item">
                    <div class="list-item-main very-wide-grid plugin-grid">
                        ...
                        <div class="plugin-cell cell-http-config wide-cell">
                            ...
                            <div class="http-grid">
                               ...
                               <label for={`plugin-method-${i}`}>{$t('pluginManager.method')}</label><select id={`plugin-method-${i}`} bind:value={plugin.http_request_config.method}><option>GET</option><option>POST</option><option>PUT</option><option>DELETE</option></select>
                               <label for={`plugin-headers-${i}`}>{$t('pluginManager.headers')}</label><textarea id={`plugin-headers-${i}`} rows=2 placeholder={'{ "Authorization": "Bearer YOUR_TOKEN" }'} bind:value={plugin.http_request_config.headers}></textarea>
                    ...
                    <button class="remove-btn" on:click={() => removePlugin(i)} title="Remove">×</button>
                </div>
                {/each}
            </div>
